...
(defn +list [& lists]
  (let [len (count (nth lists 0))]
    (map 
      (fn [pos] ;for each position
        (apply +  ;sum
               (map (fn [list] ; each element of each list
                      (nth list pos)) ;that's at that position
                    lists))) 
      (range len))))
...
(defn rel-to-pos [state x y]
  (apply abs-to-abs state (rel-to-abs x y)))
...
(defn add-whats-necessary [state pos]
  (let [old-x (get state :x)
        old-y (get state :y)
        [dx dy] pos
        new-x (+ old-x dx)
        new-y (+ old-y dy)]
...
(defn change-map [keys-down state]
  ...
  ;TODO: This function is getting pretty gnarly.
  ...
  (let [[dx dy] (get-delta keys-down)
      ...
      (cond 
        (< x 0)
          (do
            (let [state (add-whats-necessary state [(- *abs-map-width* 16) 0])]
        ...
        (< y 0)
          (do
            (let [state (add-whats-necessary state [0 (- *abs-map-width* 16)])]
              (assoc state :cur-map simple-map-2)))
...
(defn pos-from-state [state]
  (list (get state :x) (get state :y)))
...
(defn move-char [keys-down state]
  ...
  (let [[dx dy] (get-delta keys-down)
        ...
        state (if (valid-position? (+list vect-x (pos-from-state state)))
                (add-whats-necessary state vect-x)
                state)
        state (if (valid-position? (+list vect-y (pos-from-state state)))
                (add-whats-necessary state vect-y)
                state)]
    state))
...
(defn draw-tile [state gfx x y type]
  "Draws an individual tile."
  (let [[tile-x tile-y] (rel-to-pos state x y)]
    (when (visible-abs? tile-x tile-y)
      (draw-abs gfx tile-x tile-y type))))

(defn draw-tiles [state gfx]
  "Draw all tiles."
  (doseq [x (range *map-width*)
          y (range *map-width*)]
    (draw-tile state gfx x y (get-tile-type (get-tile-data x y)))))

(defn draw-characters [state gfx window]
  "Draw all characters."
  (doseq [key (keys *characters*)]
    (let [[x y] (abs-to-abs state (get state :x) (get state :y))] ;TODO: indicates easier way
      (draw-abs gfx x y (get-tile-type \c)))))

(defn draw-state [state gfx window]
  "The top level drawing function."
  (let [image (.createImage window *width* *height*)
        off-gfx (.createGraphics image)]
    (draw-tiles state off-gfx)
    (draw-characters state off-gfx window)
    (.drawImage gfx image 0 0 window)))

(defn game-loop [frame state]
  (let [gfx (.getGraphics frame)]
    (Thread/sleep *delay*)
    (draw-state state gfx frame)
...
(defn main []
  ...
  (do (game-loop (make-frame *width* *height*)
                 (hash-map :cur-map simple-map :x 30 :y 30))))
